...
#### 数组实现
+ 思路：新元素直接push， 如果已经存在，找到之后放到最后一个，满了之后删除第一个
...
#### 单链表实现   
+ 思路：新进来的插入到链表最后，到达上限删除第一个元素，匹配到元素之后，把该元素排到链表尾部
    + js 版本
        ...
        findAndReSort(node) {
            ...
            while (current.next) {
                if (current.data === data) {
                    pre.next = current.next // 匹配到了该元素 ，该元素的前一个指针的next 直接指向 该元  素的next 即可
                    temp = true
                }
                pre = pre.next
                current = current.next
    ...
    +  js 改进版 (上面每次取最后一个，需要遍历，这次用一个指针保存尾部指针即可)
